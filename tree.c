...
static int write_tree_level(IndexEntry *entries, int count, int prefix_len, ObjectID *out_id) {
    Tree tree;
    tree.count = 0;

    int i = 0;
    while (i < count) {
        const char *curr_path = entries[i].path + prefix_len;
        char *slash = strchr(curr_path, '/');
